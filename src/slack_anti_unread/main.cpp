...
bool patch_slack_app_icons(const fs::path & app_folder) {
...
void patch_all_slack_installations(const fs::path & slack_base_path) {
    ...
    for(const auto & app_folder : app_folders) {
        const bool icons_patched       = patch_slack_app_icons(app_folder);
        const bool need_to_close_slack = icons_patched && !any_icons_patched;
...
void watch_for_slack_updates(const fs::path & slack_folder) {
    ...
    auto reader = wil::make_folder_change_reader(slack_folder.c_str(), true, wil::FolderChangeEvents::All, on_change);
